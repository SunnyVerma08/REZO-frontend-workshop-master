<script>
	...
	onMount(async () => {
		...
		bookingData = (await APICall(`http://127.0.0.1:3000/api/v1/bookings`)).data.data.data;
	});

	// workshop to get in booking
	const workshopTakeIn = async (bookingId) => {
		...
		try {
			const res = await axios({
				...
				data: {
					workshopAllotedTo: $USERDATA._id
				},
				headers: $USERHEADERS
			});

			if (res.data.status === 'success') {
				notifications.success('Successfully Alloted to you', 2000);
				refreshBookings();
			}
		} catch (err) {
...
<section class="w-full pt-16 pb-20 bg-gray-50">
	<div class="px-10 mx-auto text-center max-w-7xl">
		<h2 class="text-5xl font-bold text-blue-600">
			LIVE <span class="text-gray-800">BOOKINGS</span>
		</h2>
